...
func TestIngesterUserLimitExceeded(t *testing.T) {
	...
	testLimits := func() {
		...
		assert.Equal(t, wrapWithUser(makeLimitError(ing.limiter, userID, errMaxSeriesPerUserLimitExceeded), userID).Error(), string(httpResp.Body))
...
func TestIngesterMetricLimitExceeded(t *testing.T) {
	...
	testLimits := func() {
		...
		require.True(t, ok, "returned error is not an httpgrpc response")
		assert.Equal(t, http.StatusBadRequest, int(httpResp.Code))
		assert.Equal(t, wrapWithUser(makeMetricLimitError(ing.limiter, userID, mimirpb.FromLabelAdaptersToLabels(labels3), errMaxSeriesPerMetricLimitExceeded), userID).Error(), string(httpResp.Body))
